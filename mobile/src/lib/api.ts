...
export const authEvents = {
  on(event: "sessionExpired", listener: SessionExpiredListener) {
    if (event === "sessionExpired") _sessionExpiredListeners.push(listener);
  },
  off(event: "sessionExpired", listener: SessionExpiredListener) {
  ...
  emit(event: "sessionExpired") {
    if (event === "sessionExpired") _sessionExpiredListeners.forEach(l => l());
  },
};
...
const fetchFn = async <T>(path: string, options: FetchOptions): Promise<T> => {
  ...
  try {
    ...
    if (!response.ok) {
      // If 401, session has expired — notify the app once so it can redirect to login
      if (response.status === 401 && !_sessionExpiredNotified) {
      ...
      try {
        ...
        // If parsing fails, use the status text as the error
        errorData = response.statusText;
      }

      // Throw a descriptive error with status code, status text, and server error data
      throw new Error(
        `[api.ts]: ${response.status} ${response.statusText} ${typeof errorData === 'string' ? errorData : JSON.stringify(errorData)}`,
      );
    }
